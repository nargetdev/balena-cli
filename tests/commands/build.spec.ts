...
describe('balena build', function() {
	...
	this.beforeEach(() => {
		...
		docker.expectGetInfo({ persist: true });
	...
	it('should create the expected tar stream', async () => {
		...
		expect(
			cleanOutput(out).map(line =>
				line.replace(/\s{2,}/g, ' ').replace(repoPath, ''),
			),
		).to.include.members([
			'[Info] Creating default composition with source: /tests/test-data/projects/no-docker-compose/basic',
